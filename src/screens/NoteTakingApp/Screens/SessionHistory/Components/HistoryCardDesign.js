...
const HistoryCardDesign = ({ data }) => {
  ...
  const SecondCont = ({ data }) => {
    const { EndTime, StartTime } = calculateEndTime(
      data.appointment_date_time.slice(11),
      data.time_duration
    );
    return (
      <View style={styles.SecondCont}>
        ...
        <View style={[styles.secondRow, styles.row]}>
          <View style={styles.DealsWithCont}>
            <Text style={styles.DealwithText}>Status : {data.status}</Text>
          </View>
        </View>
        <View style={[styles.fourthRow, styles.row, styles.secondRow]}>
          <View style={styles.DealsWithCont}>
            <Text style={styles.DealwithText2}>{data.appointment_date}</Text>
          </View>
        </View>
        <View>
          <View style={[styles.row, styles.firstCont, { marginTop: Wp(5) }]}>
            <Text style={[styles.SessionText]}>
              {`${StartTime} - ${EndTime} ${
                DateConstrctor(
                  new Date(data.appointment_date_time.slice(0, 10))
                ).Day
              }`}
            </Text>
          </View>
        ...
        <View>
          <View style={{ flexDirection: "row" }}>
            <View style={styles.Icon}>
              <TouchableOpacity onPress={setCheckDesign}>
                <TickIcon
