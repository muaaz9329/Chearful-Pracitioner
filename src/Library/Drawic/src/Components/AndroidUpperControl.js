...
const AndroidUpperControl = () => {
  const { Opacity, Color, strokeWidth } = useSelector(
    (state) => state.BrushControl
  );

  const [Width, setWidth] = useState(strokeWidth);
  const [opacityState, setOpacity] = useState(Opacity);
  const StateRef = React.useRef(strokeWidth);
  const StateRef2 = React.useRef(Opacity);
      ...
      <View>
        <View style={styles.StrokeWidthControl}>
          ...
          <Slider
            ...
            onSlidingComplete={(value)=>Dispatch(UpdateStrokeWidth(value))}
        ...
        <View style={styles.StrokeWidthControl}>
          ...
          <Slider
            minimumValue={0.1}
            maximumValue={1}
            value={StateRef2.current}
            onValueChange={(value) => setOpacity(value)}
            onSlidingComplete={(value)=> Dispatch(UpdateOpacity(value))}
            style={{ marginTop: Wp(10) }}
          />
        </View>
      </View>
...
//* Old component that i had changed due to that stupid sliding button of slider glitch
// import { StyleSheet, Text, View } from 'react-native'
